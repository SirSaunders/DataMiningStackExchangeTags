...
# This is for seeing how our classifier does on crypto etc.
...
FILENAME_TO_READ = 'crypto.csv'
FILENAME_TO_WRITE = 'crypto_output.csv'
STOP_WORDS = set(nltk.corpus.stopwords.words('english'))
STOP_WORDS.update(
    ['.', ',', '"', "'", '?', '!', ':', ';', '(', ')', '[', ']', '{', '}'])
POS_TO_KEEP = set(["NNS", "NN"])  # keep plural nouns and possessive nouns
CSV_FILE_HEADER = ["id", "title", "content", "tags", "algorithm_tag"]
...
def get_tags(title_str, content_str):
    ...
    if len(tags_to_keep) == 0:
        ...
        if len(most_common_tags) == 0:
            return []
        tags_to_keep.add(random.choice(most_common_tags))
        tags_to_keep.add(random.choice(most_common_tags))
...
with open(FILENAME_TO_READ, 'r') as csv_file_to_read:
    with open(FILENAME_TO_WRITE, 'w') as csv_file_to_write:
        reader = csv.DictReader(csv_file_to_read)
        writer = csv.DictWriter(csv_file_to_write, fieldnames=CSV_FILE_HEADER)
        # Loop through every row in the file
        # Convert to lowercase, split by ' ' to get a list of words
        for row in reader:
            row['title'] = row['title'].lower()
            row['content'] = re.sub(STRIP_HTML_REGEX, "", row['content']).lower()
            row['tags'] = row['tags'].lower().split(' ')
            row['algorithm_tag'] = get_tags(row['title'], row['content'])
            writer.writerow(row)
